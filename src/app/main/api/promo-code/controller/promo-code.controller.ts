...
export const createPromoCodeController = (handlers: PromoCodeControllerHandlers) => ({
  create(
    params: { id: string },
    body: { code: string; type: "Percentage" | "FixedAmount"; discountValue: number; maxUsage: number; validStart: string; validEnd: string; minPurchaseAmount?: number },
  ) {
    const command = new CreatePromoCodeCommand(
      params.id, body.code, body.type, body.discountValue, body.maxUsage,
      new Date(body.validStart), new Date(body.validEnd), body.minPurchaseAmount,
    );
    return handlers.createPromoCodeHandler.execute(command)
      .then((promoCodeId) => success({ id: promoCodeId }, "Promo code created successfully"));
  },

  list(params: { id: string }) {
    return handlers.listPromoCodesHandler.execute(new ListPromoCodesQuery(params.id))
  ...
  validate(body: { eventId: string; code: string; purchaseAmount: number }) {
    return handlers.validatePromoCodeHandler.execute(new ValidatePromoCodeCommand(body.eventId, body.code, body.purchaseAmount))
      .then((result) => success(result, result.message || "Promo code validated"));
  },

  deactivate(params: { id: string }) {
    return handlers.deactivatePromoCodeHandler.execute(new DeactivatePromoCodeCommand(params.id))
