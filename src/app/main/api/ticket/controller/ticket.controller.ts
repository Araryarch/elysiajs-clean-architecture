...
export const createTicketController = (handlers: TicketControllerHandlers) => ({
  checkIn(body: { ticketCode: string; eventId: string }) {
    return handlers.checkInHandler
      .execute(new CheckInTicketCommand(body.ticketCode, body.eventId))
  ...
  search(query: {
    ...
    return handlers.searchTicketsHandler
      .execute(
  ...
  getById(params: { id: string }) {
    return handlers.getTicketHandler
      .execute(new GetTicketQuery(params.id))
